...
<%
	...
	out.print("<br>");
	out.print("특정 문자열 are를 포함하는지 : "  + message.contains("are"));	
	out.print("<br>");
...
%>
	...
	<hr>
	<p>core, fmt 라이브러리는 태그를 제공하고 fn 라이브러리 함수를 사용할 수 있게합니다.
	문자열 메소드의 결과 값이 태그 안에서 출력되도록 합니다.</p>
	<ul>
		...
		<li><c:out value="${fn:replace(msg,'viviz','twice') }"/></li>
	</ul>
	<ul>
		<c:set value = "he         llo      !!" var="temp"/>
		<li>공백이 많은 문자열 : <c:out value="${temp }"/>(이유) html 문서에는 공백이 하나만 반영 됩니다.연속된 공백을 모두 출력할 때에는 &nbsp; 사용해야 합니다.</li>
		<li><c:out value="${fn:replace(temp,' ','&nbsp;') }"/></li>
		<li>"${fn:replace(temp,' ','&nbsp;') }"/></li>
		<li><c:set value = "${fn:split(msg,' ') }" var="temps"/></li>
	</ul>
		
	<h4>msgArr 배열 애트리뷰트 값을 하나씩 출력하기</h4>
	<ul>
	<c:forEach items="${msgArr }" var="ele">
		<li><c:out value="${ele }"/></li>
	</c:forEach>
	</ul>
	<h4>temps 배열 애트리뷰트 값을 하나씩 출력하기</h4>
	<ul>
